    ...
    <div class="relative z-10 w-full max-w-md rounded-2xl border border-white/10 bg-white/10 backdrop-blur-lg p-8 shadow-2xl">
      ...
      <?php if(session('success')): ?>
        <div class="mb-4 rounded-xl bg-green-500/20 border border-green-500/30 p-4 text-sm">
          <div class="flex items-center gap-2">
            ...
            <span class="text-green-200 font-medium"><?php echo e(session('success')); ?></span>
          </div>
        </div>
      <?php endif; ?>
      ...
      <?php if(session('status')): ?>
        <div class="mb-4 rounded-xl bg-blue-500/20 border border-blue-500/30 p-4 text-sm">
          <span class="text-blue-200"><?php echo e(session('status')); ?></span>
        </div>
      <?php endif; ?>
      ...
      <?php if(session('error')): ?>
        <div class="mb-4 rounded-xl bg-red-500/20 border border-red-500/30 p-4 text-sm">
          <div class="flex items-center gap-2">
            ...
            <span class="text-red-200"><?php echo e(session('error')); ?></span>
          </div>
        </div>
      <?php endif; ?>
      ...
      <form id="login-form" action="<?php echo e(route('login')); ?>" method="POST" class="space-y-5" novalidate>
        <?php echo csrf_field(); ?>
     ...
     <input id="email" name="email" type="email" inputmode="email" autocomplete="email" required
       ...
       class="mt-1 w-full rounded-xl border bg-black/30 px-3 py-2 text-white placeholder-gray-400 outline-none focus:ring-2 focus:ring-white/70 <?php echo e($errors->has('email') ? 'border-red-400' : 'border-white/20'); ?>">
...
unset($__errorArgs, $__bag); ?>
        ...
        <div>
          ...
          <input id="password" name="password" type="password" autocomplete="current-password" required placeholder="••••••••"
                 class="mt-1 w-full rounded-xl border bg-black/30 px-3 py-2 text-white placeholder-gray-400 outline-none focus:ring-2 focus:ring-white/70 <?php echo e($errors->has('password') ? 'border-red-400' : 'border-white/20'); ?>">
...
unset($__errorArgs, $__bag); ?>
        ...
        <div class="flex justify-end text-sm">
          <a href="<?php echo e(url('/password/reset')); ?>" class="text-gray-400 hover:text-white underline underline-offset-4">Lupa kata sandi?</a>
        </div>
  ...
  <button type="submit" id="submit-btn"
    ...
    Masuk
        ...
        <p class="text-center text-sm text-gray-400 mt-4">
          Belum punya akun? <a href="<?php echo e(route('register')); ?>" class="underline hover:text-white">Daftar</a>
        </p>
      </form>
