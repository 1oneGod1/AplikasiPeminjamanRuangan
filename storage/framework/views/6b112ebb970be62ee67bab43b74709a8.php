...
<div class="space-y-4 md:space-y-6">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    ...
    <button onclick="openCreateUserModal('')" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-500 text-white rounded-xl font-semibold hover:bg-blue-600 transition-colors shadow-lg shadow-blue-500/30">
  ...
  <div class="rounded-2xl border border-white/10 bg-linear-to-br from-white/10 to-white/5 backdrop-blur-sm p-6">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h4 class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Total User Non-Peminjam</h4>
          <p class="text-3xl font-bold text-white mt-1"><?php echo e($users->count()); ?></p>
        </div>
      </div>
    </div>
  <div class="rounded-2xl border border-white/10 bg-linear-to-br from-green-500/20 to-green-500/10 backdrop-blur-sm p-6">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h4 class="text-xs font-semibold text-green-300 uppercase tracking-wider">User Aktif</h4>
          <p class="text-3xl font-bold text-white mt-1"><?php echo e($users->where('is_active', true)->count()); ?></p>
        </div>
      </div>
    </div>
  </div>
          ...
          <tr class="hover:bg-white/5 transition-colors">
            ...
            <td class="px-4 lg:px-6 py-4">
              <div class="flex items-center justify-center gap-2">
                <button onclick="openEditUserModal(<?php echo e($u->id); ?>)" class="text-yellow-400 hover:text-yellow-300 font-medium transition-colors">Edit</button>
    ...
    <?php $__empty_1 = true; $__currentLoopData = $users; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $u): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
      <div class="rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm p-4 space-y-3">
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3 flex-1 min-w-0">
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-xs text-slate-400"><?php echo e($u->getRoleLabel()); ?></p>
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center gap-2 pt-2 border-t border-white/10">
          <button onclick="openEditUserModal(<?php echo e($u->id); ?>)" class="flex-1 text-yellow-400 hover:text-yellow-300 font-medium transition-colors text-center">Edit</button>
          <span class="text-slate-700">·</span>
