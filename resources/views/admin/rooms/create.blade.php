...
<div class="max-w-4xl mx-auto">
  ...
  <div class="{{ $adminFormCardClass }}">
    <form action="{{ isset($room) ? route('admin.rooms.update', $room) : route('admin.rooms.store') }}" method="POST" class="space-y-12">
      @csrf
      @if(isset($room))
        @method('PUT')
      @endif
      ...
      <div class="space-y-10">
        ...
        <header class="space-y-2.5">
          ...
          <h3 class="{{ $adminFormHeaderClass }}">{{ isset($room) ? 'Perbarui' : 'Tambah' }} data ruangan</h3>
          <p class="{{ $adminFormSubtextClass }}">Mohon lengkapi detail ruangan agar proses peminjaman berjalan tertib dan transparan.</p>
        </header>
        ...
        <footer class="flex flex-col gap-3 pt-6 sm:flex-row sm:items-center sm:justify-end {{ $adminFormSectionDivider }}">
          ...
          <button type="submit" class="{{ $adminPrimaryButtonClass }}">{{ isset($room) ? 'Perbarui Ruangan' : 'Simpan Ruangan' }}</button>
        </footer>
      </div>
    </form>
  </div>
