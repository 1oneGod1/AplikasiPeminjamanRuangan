        ...
        <tbody class="divide-y divide-white/5">
          @forelse($roomTypes as $type)
            <tr class="hover:bg-white/5 transition-colors">
              ...
              <td class="px-4 lg:px-6 py-4 text-center">
                <button onclick="openCreateRoomTypeModal({{ $type->id }})" class="text-yellow-400 hover:text-yellow-300 font-medium transition-colors">Edit</button>
      ...
      <div class="rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm p-4 space-y-3">
        ...
        <div class="space-y-2">
          <div class="flex items-center gap-2 text-sm text-slate-300">
            ...
            <span>Nama: <strong>{{ $type->name }}</strong></span>
          </div>
          <div class="flex items-start gap-2 text-sm text-slate-400">
            <i class="fas fa-clipboard-list text-green-400 w-4 mt-0.5"></i>
            <span class="flex-1">{{ $type->description ?? 'Tidak ada deskripsi' }}</span>
          </div>
        </div>

        <!-- Actions -->
        <div class="flex items-center gap-2 pt-2 border-t border-white/10">
          <button onclick="openCreateRoomTypeModal({{ $type->id }})" class="flex-1 text-yellow-400 hover:text-yellow-300 font-medium transition-colors text-center">Edit</button>
          <span class="text-slate-700">·</span>
          <form action="{{ route('admin.room-types.destroy', $type) }}" method="POST" class="flex-1" onsubmit="return confirm('Yakin hapus jenis ruangan ini?')">
            @csrf
            @method('DELETE')
...
@include('admin.modals.create-room-type-modal')
@endsection
