...
<div class="space-y-6">
  ...
  <div class="flex items-center justify-between">
    <div>
      ...
      <p class="text-sm text-gray-600">Kelola data peminjam ruangan yang terdaftar</p>
    </div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    <div class="bg-white rounded-xl p-6 shadow border">
      ...
      <p class="text-3xl font-bold text-gray-900 mt-2">{{ $totalPeminjam }}</p>
    </div>
    <div class="bg-white rounded-xl p-6 shadow border">
      <h4 class="text-sm font-medium text-gray-500">Peminjam Aktif</h4>
      <p class="text-3xl font-bold text-green-600 mt-2">{{ $activePeminjam }}</p>
    </div>
  </div>
        ...
        <tbody>
          @forelse($peminjam as $user)
            <tr class="border-b hover:bg-gray-50">
              <td class="px-6 py-4">{{ $loop->iteration + ($peminjam->currentPage() - 1) * $peminjam->perPage() }}</td>
