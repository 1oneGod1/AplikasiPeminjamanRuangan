...
<div class="space-y-4 md:space-y-6">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    ...
    <a href="{{ route('admin.users.create', 'peminjam') }}" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-500 text-white rounded-xl font-semibold hover:bg-blue-600 transition-colors shadow-lg shadow-blue-500/30">
  ...
  <div class="rounded-2xl border border-white/10 bg-linear-to-br from-white/10 to-white/5 backdrop-blur-sm p-6">
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <p class="text-3xl font-bold text-white mt-1">{{ $totalPeminjam }}</p>
        </div>
      </div>
    </div>
  <div class="rounded-2xl border border-white/10 bg-linear-to-br from-green-500/20 to-green-500/10 backdrop-blur-sm p-6">
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <p class="text-3xl font-bold text-white mt-1">{{ $activePeminjam }}</p>
        </div>
      </div>
    </div>
  </div>
        ...
        <tbody class="divide-y divide-white/5">
          @forelse($peminjam as $user)
            <tr class="hover:bg-white/5 transition-colors">
              <td class="px-4 lg:px-6 py-4 text-slate-400">{{ $loop->iteration + ($peminjam->currentPage() - 1) * $peminjam->perPage() }}</td>
              ...
              <td class="px-4 lg:px-6 py-4 text-slate-300">{{ $user->email }}</td>
              <td class="px-4 lg:px-6 py-4 text-slate-400">{{ $user->phone ?? '-' }}</td>
              <td class="px-4 lg:px-6 py-4">
                @if($user->is_active)
              ...
              <td class="px-4 lg:px-6 py-4">
                <div class="flex items-center justify-center gap-2">
                  <a href="{{ route('admin.users.edit', $user) }}" class="text-yellow-400 hover:text-yellow-300 font-medium transition-colors">Edit</a>
                  <span class="text-slate-700">·</span>
                  <form action="{{ route('admin.users.destroy', $user) }}" method="POST" class="inline" onsubmit="return confirm('Yakin hapus peminjam ini?')">
                    @csrf
                    @method('DELETE')
          ...
          @empty
            <tr>
              <td colspan="6" class="px-6 py-12 text-center">
                ...
                <a href="{{ route('admin.users.create', 'peminjam') }}" class="inline-flex items-center gap-2 mt-4 px-4 py-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600 transition-colors">
                  <i class="fas fa-plus"></i>
                  Tambah sekarang
                </a>
              </td>
    ...
    @forelse($peminjam as $user)
      <div class="rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm p-4 space-y-3">
        ...
        <div class="flex items-center gap-2 pt-2 border-t border-white/10">
          <a href="{{ route('admin.users.edit', $user) }}" class="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-yellow-500 text-white text-sm font-semibold hover:bg-yellow-600 transition-colors">
            <i class="fas fa-edit"></i>
            <span>Edit</span>
          </a>
          <form action="{{ route('admin.users.destroy', $user) }}" method="POST" class="flex-1" onsubmit="return confirm('Yakin hapus peminjam ini?')">
            @csrf
            @method('DELETE')
