					...
					<div class="rounded-2xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 space-y-3 shadow-sm">
						...
						<div class="flex items-start justify-between">
							...
							<span @class([
								...
								<span>{{ $booking['status'] === Booking::STATUS_APPROVED ? 'Disetujui' : 'Menunggu' }}</span>
							</span>
						</div>
						...
						<div class="space-y-2 bg-slate-50 dark:bg-slate-950 rounded-xl p-3">
							<div class="flex items-center gap-2">
								...
								<span class="font-semibold text-slate-900 dark:text-white text-sm">{{ $booking['room'] }}</span>
							</div>
							<div class="flex items-center gap-2">
								<i class="fas fa-clock text-slate-600 dark:text-slate-400 w-4"></i>
								<span class="text-sm text-slate-700 dark:text-slate-300">{{ $booking['time_range'] }}</span>
							</div>
							<div class="text-xs text-slate-600 dark:text-slate-400">
								Pengaju: <strong>{{ $booking['user'] }}</strong>
							</div>
						</div>

						<!-- Purpose -->
						@if(!empty($booking['purpose']))
							<button type="button" data-purpose-modal-trigger data-purpose="{{ e($booking['purpose']) }}" data-purpose-title="Tujuan · {{ $booking['room'] }}" class="w-full flex items-center justify-center gap-2 rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 px-3 py-2 text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition">
