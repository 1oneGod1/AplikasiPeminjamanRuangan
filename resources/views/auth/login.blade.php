    ...
    <div class="relative z-10 w-full max-w-5xl px-4">
      <div class="grid gap-10 rounded-[30px] border border-white/10 bg-slate-950/60 p-10 shadow-2xl shadow-blue-900/30 backdrop-blur-xl lg:grid-cols-[1.2fr_0.9fr]">
        ...
        <div class="space-y-6 rounded-2xl border border-white/10 bg-slate-950/55 p-8 shadow-lg shadow-black/20">
          ...
          @if (session('status'))
            <div class="rounded-xl border border-blue-500/40 bg-blue-500/15 p-4 text-sm text-blue-100 shadow-inner shadow-blue-900/30">{{ session('status') }}</div>
          @endif
          ...
          <form id="login-form" action="{{ route('login') }}" method="POST" class="space-y-6" novalidate>
            @csrf
            ...
            <div class="flex items-center justify-between text-xs uppercase tracking-[0.2em] text-slate-500">
              ...
              <a href="{{ url('/password/reset') }}" class="font-semibold text-slate-300 hover:text-white">Lupa kata sandi?</a>
            </div>
            ...
            <p class="text-center text-sm text-slate-300">Belum punya akun? <a href="{{ route('register') }}" class="font-semibold text-yellow-400 hover:text-yellow-300">Daftar</a></p>
          </form>
