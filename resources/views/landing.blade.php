...
<body class="bg-slate-950 text-slate-100">
    ...
    <header class="border-b border-white/10 bg-slate-950/50 backdrop-blur-sm sticky top-0 z-50">
        <div class="mx-auto max-w-7xl px-6 py-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    <img src="https://static.wixstatic.com/media/07639e_83549958900b44ad9fea05d99e380dd5~mv2.png/v1/fill/w_559,h_512,al_c/07639e_83549958900b44ad9fea05d99e380dd5~mv2.png" alt="SPH Logo" class="h-12 w-12 rounded-2xl border border-white/10 bg-white/10 p-1 object-contain">
                    <div>
                        <h1 class="text-xl font-bold text-white">Peminjaman Ruangan</h1>
                        <p class="text-xs text-slate-400">Sekolah Palembang Harapan</p>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    <a href="{{ route('login') }}" class="rounded-xl border border-white/20 bg-white/5 px-6 py-2.5 text-sm font-semibold text-white transition hover:bg-white/10">Masuk</a>
                    <a href="{{ route('register') }}" class="rounded-xl bg-blue-500 px-6 py-2.5 text-sm font-semibold text-white shadow-lg shadow-blue-500/30 transition hover:bg-blue-600">Daftar</a>
                </div>
            </div>
        </div>
    </header>
    ...
    <section class="relative overflow-hidden bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950/80 py-20">
        ...
        <div class="relative mx-auto max-w-6xl px-6 text-center">
            ...
            <div class="mt-10 flex flex-wrap items-center justify-center gap-4">
                <a href="{{ route('register') }}" class="rounded-xl bg-blue-500 px-8 py-3.5 text-base font-bold text-white shadow-lg shadow-blue-500/30 transition hover:bg-blue-600 hover:shadow-blue-500/50">Mulai Sekarang</a>
                <a href="{{ route('login') }}" class="rounded-xl border border-white/20 bg-white/5 px-8 py-3.5 text-base font-bold text-white backdrop-blur transition hover:bg-white/10">Lihat Detail</a>
            </div>
        </div>
    </section>
        ...
        <div class="mx-auto max-w-6xl px-6">
            <div class="grid gap-6 sm:grid-cols-3">
                <div class="rounded-3xl border border-white/10 bg-white/5 backdrop-blur-sm p-8 text-center shadow-xl">
                    <div class="text-4xl font-extrabold text-blue-400">{{ $stats['rooms'] ?? 0 }}</div>
                    <div class="mt-2 text-xs uppercase tracking-[0.35em] text-slate-400">Ruangan Aktif</div>
                </div>
                <div class="rounded-3xl border border-white/10 bg-white/5 backdrop-blur-sm p-8 text-center shadow-xl">
                    <div class="text-4xl font-extrabold text-blue-400">{{ $stats['bookings'] ?? 0 }}</div>
                    <div class="mt-2 text-xs uppercase tracking-[0.35em] text-slate-400">Total Peminjaman</div>
                </div>
                <div class="rounded-3xl border border-white/10 bg-white/5 backdrop-blur-sm p-8 text-center shadow-xl">
                    <div class="text-4xl font-extrabold text-blue-400">{{ $stats['room_types'] ?? 0 }}</div>
            ...
            <div class="grid gap-6 sm:grid-cols-2">
                @forelse ($rooms as $room)
                    <div class="rounded-3xl border border-white/10 bg-white/5 backdrop-blur-sm p-6 shadow-xl hover:bg-white/10 transition">
                        <p class="text-xs uppercase tracking-[0.35em] text-slate-400 font-semibold">{{ $room->type ?? 'Ruangan' }}</p>
                        <h3 class="mt-2 text-xl font-bold text-white">{{ $room->name }}</h3>
    ...
    <section class="bg-gradient-to-br from-blue-600 via-blue-500 to-blue-400 py-16">
        <div class="mx-auto max-w-4xl px-6 text-center">
            ...
            <div class="mt-8">
                <a href="{{ route('register') }}" class="inline-flex items-center gap-2 rounded-xl bg-white px-8 py-3.5 text-base font-bold text-blue-600 shadow-2xl transition hover:bg-slate-50">
                    Buat Akun Gratis
    ...
    <footer class="border-t border-white/10 bg-slate-950 py-8">
        <div class="mx-auto max-w-6xl px-6">
            <div class="flex flex-col items-center justify-between gap-4 text-center sm:flex-row sm:text-left">
                <p class="text-sm text-slate-400">&copy; {{ date('Y') }} Sekolah Palembang Harapan. All rights reserved.</p>
